...
class G_GIN(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, nconvs=3, dropout=0, pooling='mean', layer_name="GCNConv", **kwargs):
        super().__init__()

        self.convs = torch.nn.ModuleList([])
        LayerClass = getattr(pyg_nn, layer_name, None)
        if layer_name == "GINConv":
            self.convs.append(LayerClass(torch.nn.Linear(input_dim, hidden_dim), train_eps=True))
            for _ in range(nconvs-1):
                self.convs.append(LayerClass(torch.nn.Linear(hidden_dim, hidden_dim), train_eps=True))
        else:
            self.convs.append(LayerClass(input_dim, hidden_dim))
            for _ in range(nconvs-1):
