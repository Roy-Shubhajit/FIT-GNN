...
def generate_random_split_file(num_graphs, train_size, val_size, test_size, output_file="split.txt", seed=42):
    assert train_size + val_size + test_size == num_graphs, "Split sizes must sum to num_graphs"
    ...
    # Format: space-separated lists separated by tabs, no trailing newline
    with open(output_file, "w", encoding='utf-8') as f:
        f.write(f"{ ' '.join(map(str, train)) }\t{ ' '.join(map(str, val)) }\t{ ' '.join(map(str, test)) }")
...
def avg_num_node(name):
    dataset2avg_num_node = {'MNIST':71, 'CIFAR10':118, 'DD':285, 'MUTAG':18,
                        'NCI1':30, 'NCI109':30,'PROTEINS':19,
                        'ogbg-molhiv':26, 'ogbg-molbbbp':24, 'ogbg-molbace':34, 'AIDS': 7}
