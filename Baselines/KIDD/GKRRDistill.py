...
parser.add_argument('--dataset', type=str, default='PROTEINS',
                    ...
                    'ogbg-molbbbp', 'ogbg-molbace', 'ogbg-molhiv', 'PROTEINS', 'AIDS'],
...
parser.add_argument('--initialize', type=str, default='Herding',
                    choices=['Herding', 'KCenter', 'Random', 'RandomSplit'],
...
parser.add_argument('--layer_name', type=str, default="GCNConv",
                    choices=["GCNConv", "GATConv", "SAGEConv", "GINConv"],
                    help="Type of Layer for Model")
...
def index_of(graph, dataset):
    """Returns the index of a graph in a PyTorch Geometric dataset."""
    for i, g in enumerate(dataset):
        if g == graph:
            return i
    raise ValueError("Graph not found in dataset.")
...
def test(A_list, X_list, y_list, pooling, nclass, layer_name):
    ...
    for run in range(test_runs):
        model = G_GIN(input_dim=nfeat, hidden_dim=hidden_dim, output_dim=nclass,
                    nconvs=gnn_depth, dropout=dropout, pooling=pooling, layer_name=layer_name).to(device)
...
if __name__ == '__main__':
    ...
    if name in ['PROTEINS', 'NCI1', 'DD', 'NCI109', 'AIDS', 'PROTEINS']: # TUDataset
        ...
        #split_file = "splits/OriginalSplit/"+name+"_split.txt"
            ...
            #sys.exit()
        #given the number of graphs in the dataset, generate a random split file  which does not exit with given training, validation, and test set sizes
        split_file = "splits/OriginalSplit/"+name+"_split.txt"
        if not os.path.exists(split_file):
            print("Generating random split file for {}...".format(name))
            train_size = int(len(dataset) * 0.5)
            val_size = int(len(dataset) * 0.25)
            test_size = len(dataset) - train_size - val_size
            generate_random_split_file(len(dataset), train_size, val_size, test_size, split_file)
        ...
        print(f"Training set size: {len(training_set)}  | Validation set size: {len(val_set)} | Test set size: {len(test_set)}")
    ...
    elif initialize == "RandomSelect":
        file_name = "splits/RandomSelect/"+name+"_random_"+str(graph_per_class)+".txt"
    elif initialize == "RandomSplit":
        file_name = "splits/RandomSplit/"+name+"_RandomSplit_"+str(graph_per_class)+".txt"
        if not os.path.exists(file_name):
            print("Generating random split graph choose per class file for {}...".format(name))
            graphs = []
            #choose graph_per_class graphs per class from the training set
            for c in range(nclass):
                #choose graph_per_class graphs from the training set with class c
                class_graphs = []
                for i,s in enumerate(splits[0]):
                    if dataset[s].y.item() == c:
                        class_graphs.append(i)
                if len(class_graphs) < graph_per_class:
                    raise ValueError(f"Only {len(class_graphs)} graphs found for class {c}, but {graph_per_class} requested.")
                idx = random.sample(class_graphs, graph_per_class)
                graphs += idx
            with open(file_name, 'a', encoding='utf-8') as fout:
                fout.write(' '.join(map(str, graphs)))
    ...
    for cla in range(nclass):
        if initialize == "Herding" or initialize == "KCenter" or initialize == "Random" or initialize == "RandomSplit":
    ...
    for _ in range(num_epoch):
        ...
        for data in T_loader:
            ...
            if cnt % args.test_gap == args.test_gap-1:
                ...
                performance = test(A_S.detach(), X_S.detach(), y_S.detach(), "mean", nclass, args.layer_name)
                ...
                print()
                if performance[0] >= best_test or _ == 0:
                    overall_test_performance = performance
                    best_test = performance[0]
            cnt += 1
            if cnt == num_epoch:
                break
        if cnt == num_epoch:
    ...
    if not os.path.exists("KIDD_results.csv"):
        with open("KIDD_results.csv", "w") as f:
            f.write("dataset, graph_per_class, layer_name, test_mean, test_std\n")
            f.close()
        with open("KIDD_results.csv", "a") as f:
            f.write(f"{args.dataset}, {args.gpc}, {args.layer_name}, {overall_test_performance[0]}, {overall_test_performance[1]}\n")
