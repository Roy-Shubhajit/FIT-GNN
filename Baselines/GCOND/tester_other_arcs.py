...
class Evaluator:
    ...
    def test_gat(self, nlayers, model_type, verbose=False):
        ...
        with_bn = False
        if model_type == 'GAT':
            model = GAT(nfeat=feat_syn.shape[1], nhid=512, heads=16, dropout=0.0,
        ...
        model.fit(feat_syn, adj_syn, labels_syn, np.arange(len(feat_syn)), noval=noval, data=data,
                     train_iters=10000 if noval else 1000, normalize=True, verbose=verbose)
    ...
    def get_syn_data(self, model_type=None):
        ...
        args = self.args
        adj_syn = torch.load(f'fitgnn_saved/adj_{args.dataset}_{args.reduction_rate}_{args.seed}.pt', map_location='cuda')
        feat_syn = torch.load(f'fitgnn_saved/feat_{args.dataset}_{args.reduction_rate}_{args.seed}.pt', map_location='cuda')
    ...
    def train(self, verbose=True):
        ...
        '''for model_type in ['GCN']: #['GCN',  'GraphSage', 'SGC1', 'MLP', 'APPNP1', 'Cheby']
            res = []
            nlayer = 2
            for i in range(runs):
        ...
        for key in final_res.keys():
            train_acc, test_acc = final_res[key][0][1], final_res[key][0][0]
            train_std, test_std = final_res[key][1][1], final_res[key][1][0]
            with open('results.csv', 'a') as f:
                writer = csv.writer(f)
                writer.writerow([args.dataset, key, args.reduction_rate, args.seed,
