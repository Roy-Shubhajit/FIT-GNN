    ...
    def __init__(self, pyg_data, **kwargs):
        ...
        if hasattr(pyg_data, 'train_mask'):
            ...
            try:
                ...
                # for other datasets
                #number of classes
                self.nclass = len(np.unique(self.labels))
                num_train_nodes = 20* self.nclass
                num_val_nodes = 30* self.nclass
                num_test_nodes = n - num_train_nodes - num_val_nodes
                train_frac = num_train_nodes / n
                val_frac = num_val_nodes / n
                test_frac = num_test_nodes / n
                self.idx_train, self.idx_val, self.idx_test = get_train_val_test(
                        nnodes=n, val_size=val_frac, test_size=test_frac, stratify=self.labels)
