...
def save_results(args, all_acc, csv_file='results.csv'):
    # Sort accuracies and take mean/std after discarding first 10
    sorted_acc = np.sort(all_acc)
    mean_acc = np.mean(sorted_acc[10:])
    std_acc = np.std(sorted_acc[10:])
    print('Top 10 ave_acc: {:.4f}'.format(mean_acc), '+/- {:.4f}'.format(std_acc))
    # Prepare row
    row = [args.dataset, args.experiment, args.layer_name, args.coarsening_ratio, f"{mean_acc:.4f} +/- {std_acc:.4f}"]
    ...
    with open(csv_file, mode='a', newline='') as f:
        writer = csv.writer(f)
        if not file_exists:
            writer.writerow(['Dataset', 'Experiment', 'Layer', 'Coarsening Ratio', 'Accuracy (mean +/- std)'])
        writer.writerow(row)
print("\n")
