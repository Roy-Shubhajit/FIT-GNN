...
class DenseGNN(torch.nn.Module):
    def __init__(self, nfeat, nhid, nclass, nconvs=3, dropout=0, if_mlp=False, net_norm='none', pooling='mean', **kwargs):
        ...
        if self.mlp:
            ...
            LayerClass = getattr(pyg_nn, 'Dense'+kwargs['args'].layer_name, None)
        self.convs = nn.ModuleList([])
        if 'Dense'+kwargs['args'].layer_name == 'DenseGINConv':
            self.convs.append(LayerClass(torch.nn.Sequential(torch.nn.Linear(nfeat, nhid), torch.nn.ReLU(), torch.nn.Linear(nhid, nhid), torch.nn.ReLU()), train_eps=True))
            for i in range(nconvs - 1):
                self.convs.append(LayerClass(torch.nn.Sequential(torch.nn.Linear(nhid, nhid), torch.nn.ReLU(), torch.nn.Linear(nhid, nhid), torch.nn.ReLU()), train_eps=True))
        else:
            self.convs.append(LayerClass(nfeat, nhid))
            for i in range(nconvs - 1):
                self.convs.append(LayerClass(nhid, nhid))
