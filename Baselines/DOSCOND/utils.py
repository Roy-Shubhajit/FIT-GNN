...
class Dataset:
    ...
    def __init__(self, args):
        ...
        if name in ['DD', 'MUTAG', 'NCI1', 'PROTEINS', 'AIDS']:
            ...
            val_dataset = dataset[int(0.5 * len(dataset)):int(0.75 * len(dataset))]
...
def get_mean_nodes(args):
    ...
    if args.dataset == 'AIDS':
        dataset = TUDataset(osp.join(osp.dirname(osp.realpath(__file__)), 'data', 'AIDS'), name='AIDS', transform=T.Compose([Complete()]), use_node_attr=True)
        mean_nodes = np.mean([g.num_nodes for g in dataset])
        median_nodes = np.median([g.num_nodes for g in dataset])
        class_means = []
        for c in range(dataset.num_classes):
            #print("mean nodes for class", c, ":", np.mean([g.num_nodes for g in dataset if g.y.item() == c]),"min nodes for class", c, ":", np.min([g.num_nodes for g in dataset if g.y.item() == c]), "and number of graphs:", len([g.num_nodes for g in dataset if g.y.item() == c]))
            class_means.append(np.mean([g.num_nodes for g in dataset if g.y.item() == c]))
            #if np.mean([g.num_nodes for g in dataset if g.y.item() == c]) < mean_nodes:
                #mean_nodes = np.mean([g.num_nodes for g in dataset if g.y.item() == c])
        #print("mean nodes for AIDS dataset:", mean_nodes, "and median nodes:", median_nodes)
        #print("mean nodes for each class:", class_means)
        return int(np.mean(class_means))
            
    if args.dataset == 'PROTEINS':
        dataset = TUDataset(osp.join(osp.dirname(osp.realpath(__file__)), 'data', 'PROTEINS'), name='PROTEINS', transform=T.Compose([Complete()]), use_node_attr=True)
        mean_nodes = np.mean([g.num_nodes for g in dataset])
        return int(mean_nodes)
    if args.dataset == 'MUTAG':
        return 18
    if args.dataset == 'NCI1':
        return 30
