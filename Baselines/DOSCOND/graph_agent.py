...
from models_gcn import GNN
from models import DenseGNN
...
def state_dict_match(dest_state_dict, source_state_dict):
    corrected_state_dict = OrderedDict()
    for key, value in source_state_dict.items():
        if key in dest_state_dict:
            corrected_state_dict[key] = value.reshape(dest_state_dict[key].shape)
...
class GraphAgent:
    ...
    def __init__(self, data, args, device, nnodes_syn=75):
        ...
        if args.init == 'real':
            for c in range(nclass):
                ...
                if args.use_max_nodes_size:
                    feat_real, adj_real = self.get_graphs(c, batch_size=ind[1]-ind[0], max_node_size=nnodes_syn, to_dense=True)
                else:
                    feat_real, adj_real = self.get_graphs(c, batch_size=ind[1]-ind[0], to_dense=True)
    ...
    def get_graphs(self, c, batch_size, max_node_size=None, to_dense=False, idx_selected=None):
        ...
        #print("self.nnodes_all[self.real_indices_class[c]]:", self.nnodes_all[self.real_indices_class[c]])
        #print("max node size:", max_node_size)
        print('sampled:', len(sampled), 'from class:', c)
    ...
    def train(self):
        ...
        for it in range(args.epochs):
            ...
            model_syn = DenseGNN(nfeat=dataset.num_features, nhid=args.hidden, net_norm=args.net_norm, pooling=args.pooling,
                        dropout=0.0, nclass=dataset.num_classes, nconvs=args.nconvs, args=args).to(self.device)
            model_real = GNN(nfeat=dataset.num_features, nhid=args.hidden, net_norm=args.net_norm, pooling=args.pooling,
                        dropout=0.0, nclass=dataset.num_classes, nconvs=args.nconvs, args=args).to(self.device)
        
            #print(model_syn.state_dict()['convs.0.att_src'], type(model_syn.state_dict()))
            model_real_state = state_dict_match(dest_state_dict = model_real.state_dict(), source_state_dict = model_syn.state_dict())
            model_real.load_state_dict(model_real_state)
            ...
            if (it+1) % print_freq == 0:
                ...
                if not osp.exists('doscond_results.csv'):
                    with open("doscond_results.csv", 'w') as f:
                        f.write('dataset, ipc, layer_name, test_acc, test_std\n')
                
                with open("doscond_results.csv", 'a') as f:
                    f.write(f'{args.dataset}, {args.ipc}, {args.layer_name}, {res.mean(0)[2]}, {res.std(0)[2]}\n')


    def test(self, epochs=500, save=False, verbose=False, new_data=None):
        dataset = self.data[0]
        args = self.args
        model_syn = DenseGNN(nfeat=dataset.num_features, nhid=args.hidden, dropout=args.dropout, net_norm=args.net_norm,
                        nconvs=args.nconvs, nclass=dataset.num_classes, pooling=args.pooling, args=args).to(self.device)
        model_real = GNN(nfeat=dataset.num_features, dropout=0.0, net_norm=args.net_norm,
    ...
    def test_pyg_data(self, syn_data=None, epochs=500, save=False, verbose=False):
        ...
        args = self.args
        use_val = True
        model = GNN(nfeat=dataset.num_features, nconvs=args.nconvs, nhid=args.hidden, nclass=dataset.num_classes, net_norm=args.net_norm, pooling=args.pooling, dropout=args.dropout, args=args).to(self.device)
        lr = 0.001
